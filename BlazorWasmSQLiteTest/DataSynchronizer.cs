...
internal class DataSynchronizer
{
    ...
    {
        IJSObjectReference? module = await js.InvokeAsync<IJSObjectReference>("import", "./dbstorage.js");
        ...
        using ClientSideDbContext? db = await dbContextFactory.CreateDbContextAsync();
    ...
    private async Task EnsureSynchronizingAsync()
    {
        ...
        try
        {
            ...
            // Get a DB context
            using ClientSideDbContext? db = await GetPreparedDbContextAsync();
    ...
    //private void BulkInsert(DbConnection connection, IEnumerable<Todo> parts)
    //{
